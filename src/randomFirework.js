...
function geneateFirework(coords)
{
    ...
    {
        //get random number from 1 to 6
        AmountOfColours = Math.floor(Math.random() * 2) + 1
        //for amount of colours get a ranom colour and push it to colors
        for(i = 0; i < AmountOfColours; i++)
        {
            Colors.push(DataColor[Math.floor(Math.random() * DataColor.length)])
        ...
        stringExport = `summon firework_rocket ${coords} {LifeTime:${LifeTime},FireworksItem:{id:firework_rocket,Count:1,tag:{Fireworks:{Flight:${Flight},Explosions:[{Type:${Type},Flicker:${Flicker},Trail:${Trail},Colors:[I;${ColorsString}]}]}}}}`
        // console.log(stringExport)
